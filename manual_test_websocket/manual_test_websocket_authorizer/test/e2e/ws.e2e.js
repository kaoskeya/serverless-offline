...
describe('serverless', ()=>{
  describe('with WebSocket support', ()=>{
    ...
    it('should open a WebSocket with good auth', async ()=>{
      const ws=await createWebSocket({headers:{Auth:'allow'}});//`${now}`}});
    ...
    it('should get 401 when trying to open WebSocket with no auth',  async ()=>{
      let res=await req.get('').set('Sec-WebSocket-Version', '13').set('Sec-WebSocket-Key', 'tqDb9pU/uwEchHWtz91LRA==').set('Connection', 'Upgrade').set('Upgrade', 'websocket').set('Sec-WebSocket-Extensions', 'permessage-deflate; client_max_window_bits');//.set('Authorization', user.accessToken);
      expect(res).to.have.status(401);
    }).timeout(timeout);

    it('should get 403 when trying to open WebSocket with correct auth',  async ()=>{
      let res=await req.get('').set('Sec-WebSocket-Version', '13').set('Sec-WebSocket-Key', 'tqDb9pU/uwEchHWtz91LRA==').set('Connection', 'Upgrade').set('Upgrade', 'websocket').set('Sec-WebSocket-Extensions', 'permessage-deflate; client_max_window_bits').set('Auth', `${notNow}`);
